...
> 第五讲 · 动态规划 · 线性 DP / 背包 DP / 区间 DP / 状态压缩 DP
...
> **摘要**：背包问题是动态规划中的经典问题类型，研究在容量限制下如何选择物品使总价值最大。本文详细介绍了01背包、完全背包、多重背包、分组背包等多种变体，并提供了相应的解题思路与代码实现。
...
（1）状态定义。`dp[i][j]` 表示：从前 `i` 件物品中任意选择，放入容量为 `j` 的背包中所能获得的最大价值。
...
缺点：当 `s[i]` 很大时，物品总数 `Σs[i]` 会非常大，导致超时。时间复杂度为 O(V * Σs[i])。
...
朴素解法的问题在于对 `k` 的枚举是线性的。我们可以用二进制思想来优化。
任何一个整数 `s` 都可以被拆分为 `1, 2, 4, ..., 2^p` 以及一个余数 `r` 的和。例如 `s=13` 可以拆分为 `1+4+8` (13 = 1101b) 或者是 `1+2+4+6`。通过这些拆分出的数字的组合，可以凑出 `1` 到 `s` 之间的任何整数。
...
| 遍历顺序               | 顺序+顺序 | 顺序+倒序                                      | 倒序+顺序                                     | 倒序+倒序 |
| ---------------------- | --------- | ---------------------------------------------- | --------------------------------------------- | --------- |
| 先遍历物品，再遍历背包 | ✅         | ❌                                              | ✅（自己测试的，可行，没有找到网上对应的讲解） | ❌         |
| 先遍历背包，再遍历物品 | ✅         | ✅ （自己测试的，可行，没有找到网上对应的讲解） | ❌                                             | ❌         |
...
| **完全背包**     | 每种物品**可选无限次**     | 先物品，后**正序**背包 | 允许物品被重复选取         |
...
### 2.2 最长上升子序列 (largest increased sequence, LIS)
...
* `tails[k]` 表示 **长度为 k+1 的递增子序列** 的最小结尾元素。
* `tails` 始终 **递增**。
* 每遇到新元素 `x`：

  * 若 `x` 比所有结尾都大，直接加到 `tails` 后面；
  * 否则，用 `x` **替换掉第一个 >= x 的元素**（用二分查找），表示找到一个更优（更小结尾）的同长度子序列。

这样 `tails.size()` 就是当前最长上升子序列的长度。
...
| ------------------ | ---------- | ----- | --------------- | ------- |
| DP O(n²)           | O(n²)      | O(n)  | 简单易理解，直接比较      | n ≤ 1e3 |
| 贪心 + 二分 O(n log n) | O(n log n) | O(n)  | 高效，利用tails记录最优解 | n ≤ 1e5 |
...
定义：
`dp[i][j]` 表示 **将 word1 的前 i - 1 个字符** 转换成 **word2 的前 j - 1 个字符** 所需的最少操作数。

---

（2）确定递推公式

设当前考虑字符 `word1[i-1]` 与 `word2[j-1]`：

* **情况 1：** 如果 `word1[i-1] == word2[j-1]`，那么最后一个字符相同，不需要额外操作：`dp[i][j] = dp[i - 1][j - 1]`
* **情况 2：** 如果 `word1[i-1] != word2[j-1]`，我们有三种可能的操作：
  1. **插入**：让 `word1[0..i-1]` 变成 `word2[0..j-2]`，然后插入 `word2[j-1]`
     --> `dp[i][j - 1] + 1`
  2. **删除**：删除 `word1[i-1]` -- >`dp[i - 1][j] + 1`
...
转移时枚举最后一次合并的位置 (k)，即把区间 ([i, j]) 拆成 ([i, k]) 和 ([k+1, j])：
$$
dp[i][j] = \min_{i \le k < j} (dp[i][k] + dp[k+1][j] + sum[j] - sum[i-1])
$$


其中 `sum[j] - sum[i-1]` 是第 `i` 到 `j` 堆石子的总质量（即最后合并的代价）。

初始状态：`dp[i][i] = 0`，表示单独一堆不需要合并。

最终答案是 `dp[1][n]`。

时间复杂度 ($O(n^3)$)，对 ($n \le 300$) 可以接受。
...
![image-20251113221744936](assets/image-20251113221744936.png)
...
状态压缩动态规划（State Compression DP）是一种利用二进制位表示状态的技巧，常用于处理**棋盘覆盖、铺砖、格子选择**等具有“局部依赖”性质的问题。当问题的状态维度较高但每个维度取值较少（如只有 0/1）时，可以将整个状态压缩为一个整数（通常是 `int` 或 `long long`），从而在时间和空间上实现高效转移。
...
用大小为 `2×1` 的小矩形（竖或横放）来填满一个 `h × w` 的大矩形，求所有不同铺法数量。
...
1、**合理设计状态**：用二进制位表示格子的占用/空闲情况；

2、**预处理合法性**：提前判断哪些状态组合是可行的；

3、**清晰的转移逻辑**：确保状态之间能正确衔接。
...
从起点 `0` 到终点 `n-1`，不重不漏经过每个点恰好一次的最短路径，即最短 Hamilton Path。

数据范围 `n ≤ 20`，因此总路径数约为 `20!`，无法暴力，需要使用 **状态压缩 DP**。
...
这是一个典型的旅行商问题（TSP），由于 `n <= 20`，可以使用状态压缩 DP 解决。我们用一个整数的二进制表示访问过的节点集合，从而将问题转化为在不同状态间的最小代价转移。

1、**状态定义**： 
设 `f[i][j]` 表示当前已经访问过的节点集合为 `i`（以二进制形式表示），且当前位于节点 `j` 的情况下，能够得到的最短路径长度。

2、**状态转移方程**： 
对于每个状态 `i` 和每个节点 `j`，若节点 `k` 已经被访问过，则可以通过 `k` 更新 `f[i][j]` 的值。具体来说，如果 `k` 是最后一个到达 `j` 的前驱节点，则有：
`f[i][j] = min(f[i][j], f[i - (1 << j)][k] + dist[k][j])`
其中，`i - (1 << j)` 表示从未访问 `j` 的状态转移到已访问 `j` 的状态。

3、**最终答案**

要找的是：所有点都访问过且结束点是 `n-1`，因此：`答案 = f[(1<<n) - 1][n-1]`
